...
test('kartoitus, oma ääni, hyväksyntä ja pelkkä CV sekä vanhentuneen tekstin merkintä', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('status').filter({ hasText: 'Käsitellään kartoitusta' })).toBeVisible();
  ...
  await expect(card.getByRole('status').filter({ hasText: 'Kirjoitetaan' })).toBeVisible();
  await expect(card.getByText('Testi Henkilö — Interim CEO')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Jatka katsomaan tulokset' })).toHaveCount(0);
  await expect(page.getByRole('heading', { name: 'Kirjoittajat', exact: true })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Aloita alusta', exact: true }).click();
